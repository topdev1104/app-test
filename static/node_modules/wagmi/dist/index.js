...
function useBaseQuery(options, Observer) {
    ...
    useSyncExternalStore2(
        React2.useCallback(
            (onStoreChange) => isRestoring ? () => void 0 : observer.subscribe(notifyManager.batchCalls(onStoreChange)), [observer, isRestoring]
        ),
...
import {
    useQueryClient as useQueryClient_
} from "@tanstack/react-query";
...
} from "use-sync-external-store/shim/with-selector.js";

function usePublicClient({
    chainId
} = {}) {
    return useSyncExternalStoreWithSelector(
        (cb) => watchPublicClient({
            chainId
        }, cb),
        () => getPublicClient({
            chainId
...
import {
    useSyncExternalStoreWithSelector as useSyncExternalStoreWithSelector2
} from "use-sync-external-store/shim/with-selector.js";
...
import {
    useSyncExternalStoreWithSelector as useSyncExternalStoreWithSelector3
} from "use-sync-external-store/shim/with-selector.js";
...
function useAccount({
    ...
    const watchAccount_ = React9.useCallback(
        (callback) => watchAccount(callback), [config]
    );
...
function useNetwork() {
    ...
    const watchNetwork_ = useCallback5(
        (callback) => watchNetwork(callback), [config]
    );
...
import {
    replaceEqualDeep as replaceEqualDeep2
} from "@tanstack/react-query";
import {
...
import {
    replaceEqualDeep as replaceEqualDeep3
} from "@tanstack/react-query";
import {
